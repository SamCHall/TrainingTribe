...
const TotalReps = ({members}) => {
    const sortMembersByTotalReps = () => {
        const sortedMembers = members.sort((a, b) => User.getTotalReps(b) - User.getTotalReps(a));
  ...
  return (
    <View style={{flex:1}}>
          <View style={[globalStyles.leaderboardEntry, {borderTopWidth:0, borderBottomWidth:3}]}>
            ...
            renderItem={({ item, index }) => <MyTribeLeaderboard user={item} index={index} metric='reps'/>}
